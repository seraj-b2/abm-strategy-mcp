...
import {
  ...
  checkKnownSchema,
  recordFileInput,
  recordFileInputSchema,
  recordTextInput,
  recordTextInputSchema,
} from "./input.js";
...
const stageEnum = { type: "string", enum: STAGE_ORDER as unknown as string[] };

export const TOOLS = [
  {
    name: "create_project",
    description:
      "Create a new client workspace (folder tree + project.json) for a new ABM strategy project. Fails if the client's slug already exists — use get_project_status to resume an existing one instead.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Get the full project state for a client: project_inputs, input_files, all 21 stage gates, current_skill, and the computed next_gate. Always call this rather than relying on conversation memory, especially after any gap in the session.",
    inputSchema: {
      type: "object",
  ...
  },
  {
    name: "advance_stage",
    description:
      "Compute the next stage to run: the first of the 21 stages whose gate is not 'completed'. Returns is_resume=true if that stage has a partial draft already in staging (do not restart it from scratch). Call this to find what to do next, and again after each stage's approve_draft succeeds.",
    inputSchema: {
      type: "object",
      properties: { client_slug: { type: "string" } },
      required: ["client_slug"],
  ...
  },
  {
    name: "set_stage_gate",
    description:
      "Manually override a stage gate's status. For repair/recovery only — normal flow advances gates via create_draft (-> partial) and approve_draft (-> completed), not this tool.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description:
      "Persist a piece of text input the user gave you (an answer to a question, a choice between options, a proprietary data point) so later stages don't re-ask it. Always call check_known or get_stage_context first to avoid re-asking something already recorded.",
    inputSchema: {
      type: "object",
      properties: {
        client_slug: { type: "string" },
  ...
  },
  {
    ...
    description:
      "Move a file the user provided into the project's inputs/ folder and record it in project.json and input-log.md.",
    inputSchema: {
      type: "object",
      properties: {
        client_slug: { type: "string" },
  ...
  },
  {
    name: "check_known",
    description:
      "Check whether one or more pieces of information are already known (in project_inputs, text-input.md, or inputs/ filenames) before asking the user. Prefer get_stage_context for a full stage's inputs; use this for ad hoc checks mid-stage.",
    inputSchema: {
      type: "object",
      properties: {
        client_slug: { type: "string" },
  ...
  },
  {
    name: "create_draft",
    description:
      "Write a stage's first draft (v1) to staging/, setting its gate to 'partial'. Fails if a staged draft already exists for this stage — use edit_draft or redo_draft instead.",
    inputSchema: {
      type: "object",
      properties: {
        client_slug: { type: "string" },
  ...
  },
  {
    name: "edit_draft",
    description:
      "Revise the staged draft in place (same version, no archive). Use when the user asks for changes to the current draft rather than a full regeneration.",
    inputSchema: {
      type: "object",
      properties: {
        client_slug: { type: "string" },
  ...
  },
  {
    name: "redo_draft",
    description:
      "Regenerate a stage's draft as a new version: archives the current staged file, then writes the new content as the next version in staging/. Use when the user wants a different direction, not just edits to the current one.",
    inputSchema: {
      type: "object",
      properties: {
        client_slug: { type: "string" },
  ...
  },
  {
    name: "approve_draft",
    description:
      "Finalize a stage: move its staged draft to outputs/, verify the move succeeded (staging empty, outputs populated), and only then set the gate to 'completed'. If verification fails, the gate is left as-is and an actionable error is returned — do not treat the stage as done in that case.",
    inputSchema: {
      type: "object",
      properties: { client_slug: { type: "string" }, stage: stageEnum },
      required: ["client_slug", "stage"],
  ...
  },
  {
    ...
    description:
      "Log an assumption made during a stage's analytical work — including scoping choices, interpretations, defaults, estimates, and inferences, not only missing-data fallbacks. Call this as many times as needed while doing the stage's Process work.",
    inputSchema: {
      type: "object",
      properties: {
        client_slug: { type: "string" },
