...
function stagingPath(slug: string, stage: string): string {
  return path.join(subDir(slug, "staging"), STAGE_FILENAMES[stage as keyof typeof STAGE_FILENAMES]);
}

function outputsPath(slug: string, stage: string): string {
  return path.join(subDir(slug, "outputs"), STAGE_FILENAMES[stage as keyof typeof STAGE_FILENAMES]);
}

function archivePath(slug: string, stage: string, version: number, ts: string): string {
  const filename = STAGE_FILENAMES[stage as keyof typeof STAGE_FILENAMES];
...
export async function createDraft(args: z.infer<typeof createDraftSchema>) {
...
export async function getDraft(args: z.infer<typeof getDraftSchema>) {
  if (!(await projectExists(args.client_slug))) {
    return { error: `No project found for client_slug "${args.client_slug}".` };
  }
  const state = await readProject(args.client_slug);
...
export async function editDraft(args: z.infer<typeof editDraftSchema>) {
  if (!(await projectExists(args.client_slug))) {
    return { error: `No project found for client_slug "${args.client_slug}".` };
  }
  const state = await readProject(args.client_slug);
...
export async function redoDraft(args: z.infer<typeof redoDraftSchema>) {
  if (!(await projectExists(args.client_slug))) {
    return { error: `No project found for client_slug "${args.client_slug}".` };
  }
  const state = await readProject(args.client_slug);
  ...
  await appendLog(
    args.client_slug,
    "project-log.md",
    `[${ts}] ${args.stage}: v${currentVersion} moved staging -> archive${args.reason ? ` (reason: ${args.reason})` : ""}`
  );
...
export async function approveDraft(args: z.infer<typeof approveDraftSchema>) {
  if (!(await projectExists(args.client_slug))) {
    return { error: `No project found for client_slug "${args.client_slug}".` };
  }
  const state = await readProject(args.client_slug);
...
export async function recordAssumption(args: z.infer<typeof recordAssumptionSchema>) {
  if (!(await projectExists(args.client_slug))) {
    return { error: `No project found for client_slug "${args.client_slug}".` };
  }
  const ts = timestamp();
