import { z } from "zod";
import { STAGE_ORDER, type GateStatus } from "../lib/types.js";
...
export async function advanceStage(args: z.infer<typeof advanceStageSchema>) {
...
export async function setStageGate(args: z.infer<typeof setStageGateSchema>) {
  if (!(await projectExists(args.client_slug))) {
    return { error: `No project found for client_slug "${args.client_slug}".` };
  }
  const state = await readProject(args.client_slug);
  const previous: GateStatus = state.stage_gates[args.stage];
  state.stage_gates[args.stage] = args.status;
  await writeProject(state);
  await appendLog(
    args.client_slug,
    "project-log.md",
    `[${timestamp()}] MANUAL OVERRIDE: stage gate "${args.stage}" changed ${previous} -> ${args.status}`
  );
