...
import type { TextInputEntry } from "./types.js";
import { subDir } from "./workspace.js";

function textInputPath(slug: string): string {
  return path.join(subDir(slug, "inputs"), "text-input.md");
}

export async function readTextInputs(slug: string): Promise<TextInputEntry[]> {
  let raw: string;
  try {
...
export async function appendTextInput(slug: string, entry: TextInputEntry): Promise<void> {
  const block = `[${entry.timestamp}] (${entry.skill})\nQ: ${entry.question}\nA: ${entry.answer}\n---\n`;
  await fs.appendFile(textInputPath(slug), block, "utf-8");
}

export async function listInputFiles(slug: string): Promise<string[]> {
  try {
...
export async function listOutputFiles(slug: string): Promise<string[]> {
  try {
