...
import { appendLog, clientDir, listClientSlugs, logPath, nextGate, projectExists, readProject, slugify, subDir, timestamp, writeProject, } from "../lib/workspace.js";
const emptyGates = () => Object.fromEntries(STAGE_ORDER.map((s) => [s, "pending"]));
...
export async function createProject(args) {
    ...
    for (const dir of ["inputs", "staging", "archive", "outputs"]) {
    ...
    for (const name of ["input-log.md", "assumptions.md", "project-log.md"]) {
        await fs.writeFile(logPath(slug, name), `# ${name}\n\nProject: ${args.client_name} (${slug})\nCreated: ${state.created}\n\n`, "utf-8");
    }
...
export async function getProjectStatus(args) {
