...
export async function recordTextInput(args) {
...
export async function recordFileInput(args) {
    if (!(await projectExists(args.client_slug))) {
        return { error: `No project found for client_slug "${args.client_slug}".` };
    }
...
export async function checkKnown(args) {
    if (!(await projectExists(args.client_slug))) {
        return { error: `No project found for client_slug "${args.client_slug}".` };
    }
    const state = await readProject(args.client_slug);
    ...
    for (const key of args.keys) {
        ...
        const projectInputMatch = Object.entries(state.project_inputs).find(([k, v]) => normalize(k).includes(lowerKey) && v);
        if (projectInputMatch) {
            result[key] = { known: true, source: "project_inputs", value: projectInputMatch[1] };
            continue;
        }
        const textMatch = textInputs.find((t) => normalize(t.question).includes(lowerKey) || normalize(t.answer).includes(lowerKey));
        if (textMatch) {
